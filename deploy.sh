...
#############################################
# 一键部署脚本 v2 - websocket-platform & q_agent
...
WEBSOCKET_PORT=8088
QAGENT_PORT=8089
MYSQL_HOST="49.233.105.26"
MYSQL_PORT=3306
MYSQL_USER="root"
MYSQL_PASSWORD=""
MYSQL_DATABASE="q_agent"
LLM_PROVIDER="openai"
...
deploy_websocket() {
    ...
    # 同步代码
    log_info "同步代码到服务器..."
    rsync_to_server "${SCRIPT_DIR}/websocket-platform/" "${WEBSOCKET_DIR}/"
...
config['database']['host'] = '${MYSQL_HOST}'
config['database']['port'] = ${MYSQL_PORT}
config['database']['user'] = '${MYSQL_USER}'
config['database']['password'] = '${MYSQL_PASSWORD}'
config['database']['database'] = '${MYSQL_DATABASE}'
...
[Unit]
Description=Q-Agent API Service
...
ExecStart=${QAGENT_DIR}/venv/bin/python -m q_agent.api.main
Restart=always
RestartSec=5
...
show_logs() {
    ...
    case $service in
        websocket|websocket-platform|w)
            ssh_exec "journalctl -u websocket-platform -f --no-pager -n ${lines}"
            ;;
        qagent|q-agent|q)
            ssh_exec "journalctl -u q-agent -f --no-pager -n ${lines}"
            ;;
        all|*)
            ssh_exec "journalctl -u websocket-platform -u q-agent -f --no-pager -n ${lines}"
            ;;
    esac
}
...
show_help() {
    echo "
${GREEN}一键部署脚本 - websocket-platform & q_agent${NC}
...
${YELLOW}配置:${NC}
  请复制 deploy.env.example 为 deploy.env 并修改配置:
  ${BLUE}cp deploy.env.example deploy.env${NC}
